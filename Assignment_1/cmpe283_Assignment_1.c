...
#define IA32_VMX_PINBASED_CTLS 0x48D
...
detect_vmx_features(void)
{
	...
	pr_info("Pinbased Controls MSRSW: 0x%llx\n",
	...
	pr_info("Process Based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased, 21, lo, hi);

	/* Secondary Process Based controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Secodnary Process Based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
